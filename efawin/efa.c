...
extern
#endif
const struct verbs_context_ops efa_ctx_ops = {
...
struct verbs_context *efa_alloc_context(struct ibv_device *vdev,
					       int cmd_fd,
					       void *private_data)
{
	struct efa_alloc_ucontext_resp resp = {0};
	struct efa_alloc_ucontext cmd = {0};
	...
	if (!ctx)
		return NULL;
#ifdef _WIN32
       // Initialize device ourselves since there is no verbs framework on Windows
	ctx->ibvctx.context.device = vdev;
#endif
...
void efa_free_context(struct ibv_context *ibvctx)
{
...
bool is_efa_dev(struct ibv_device *device)
{
    ...
    // No verbs framework support - we can check device name instead of comparing ops.
	return !strncmp(device->dev_name,"EFA",3);
}
