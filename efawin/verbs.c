			...
			size_t attr_size)
{
	...
	struct efa_query_device_ex_resp resp = {0};
...
{
	struct efa_alloc_pd_resp resp = {0};
...
{
	struct efa_context *ctx = to_efa_context(ibvctx);
	struct efa_create_cq_resp resp = {0};
				...
				struct efadv_qp_init_attr *efa_attr)
{
	...
	struct efa_create_qp_resp resp = {0};
	struct efa_create_qp req = {0};
		   ...
		   uint32_t inlen)
{
	...
	if (!vext_field_avail(struct efadv_ah_attr, ahn, inlen))
		return EINVAL;
...
{
	struct efa_create_ah_resp resp = {0};
	struct efa_ah *ah;
	int err;
