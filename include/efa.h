...
#define EFA_SET(ptr, mask,value) { \
	      ...
	      uint64_t result = _BitScanForward64(&_index, mask##_MASK);   \
             *(ptr) = (*ptr & ~(mask##_MASK)) |                             \
	      ((value) << _index) & (mask##_MASK);                   \
	}
#endif
...
struct efa_dev {
	...
	uint32_t pg_sz;
#if _WIN32
	struct EFA_WIN_DEVICE device;
	struct EFA_DEVICE_INFO dev_attr;
#endif
};
